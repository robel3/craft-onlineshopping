...
class Attendance extends CI_Controller
{
     ...
     public function Approve_Attendance()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $id               = $this->input->get('A');
            if (!empty($id)) {
                $data['attval'] = $this->attendance_model->update_timesheet_status($id);
            }
            $data['attendancelist'] = $this->attendance_model->getAllAttendance();
            $this->load->view('backend/attendance', $data);
        } else {
      ...
      public function Emp_Save_Attendance()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $data['employee'] = $this->employee_model->emselect();
            $id               = $this->input->get('A');
            ...
            // $data['attendancelist'] = $this->attendance_model->em_attendance();
            $this->load->view('backend/emp_add_attendance', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    public function Emp_Attendance()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $id = $this->session->userdata('user_login_id');
             $b = $this->employee_model->GetBasic($id)->em_code;
            $data['attendancelist'] = $this->attendance_model->getEmpAttendance($b);
