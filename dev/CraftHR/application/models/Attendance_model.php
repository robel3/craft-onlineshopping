...
class Attendance_model extends CI_Model
{
    ...
    public function em_attendanceFor($id)
    {
     ...
     public function update_timesheet_status($id)
    {
      // $sql    = "SELECT `attendance`.*,
      // `employee`.`em_id`,`first_name`,`attendance`.`timesheet_status`,`last_name`,`em_code`
      // FROM `attendance`
      // LEFT JOIN `employee` ON `attendance`.`emp_id`=`employee`.`em_code` 
      // WHERE `attendance`.`id`='$id'";
      //   $query  = $this->db->query($sql);
      //   $stat="1";
      //   $this->db->update('timesheet_status',$stat);
      //   $result = $query->row();
      //   return $result;
      $data = array('timesheet_status' => '1');
      $this->db->select('*');
    ...
    {
      $sql = "SELECT `attendance`.`id`, `emp_id`, `atten_date`,`timesheet_status`, `signin_time`, `signout_time`, TRUNCATE(ABS(( TIME_TO_SEC( TIMEDIFF( `signin_time`, `signout_time` ) ) )/3600), 1) AS Hours,
    ...
    public function getEmpAttendance($emps_id)
    {
      $sql = "SELECT `attendance`.`id`, `emp_id`, `atten_date`,`timesheet_status`, `signin_time`, `signout_time`,  TRUNCATE(ABS(( TIME_TO_SEC( TIMEDIFF( `signin_time`, `signout_time` ) ) )/3600), 1) AS Hours,
        CONCAT(`first_name`, ' ', `last_name`) AS name
       FROM `attendance`
        LEFT JOIN `employee` ON `attendance`.`emp_id` = `employee`.`em_code`
        WHERE `attendance`.`status` = 'A' AND `employee`.`em_code` = '$emps_id' ";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
      // $sql = "SELECT * from 'attendance' where 'emp_id' = '$emps_id'" ;
      // $query  = $this->db->query($sql);
      //   $result = $query->result();
      //   return $result;
    }
    public function deleteEmp($emps_id){
      $sql = "DELETE * from `employee` where `employee`.`em_code` = '$emps_id' ";
